    ...
    public class ClassInstance : CustomTypeDescriptor
    {
        ...
        public ClassInstance(Type wrappedType, Type objectInstanceType)
        {
            ...
            _classProperties = _wrappedType
                .GetProperties()
                .Select(p => ClassMemberObservable.Intercept(new ClassMemberProperty(p, objectInstanceType)))
                .Concat<ClassMember>(_wrappedType
                    .GetFields()
                    .Select(f => ClassMemberObservable.Intercept(new ClassMemberField(f, objectInstanceType))))
                .ToList();
